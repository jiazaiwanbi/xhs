...
export default function VideoDetailView() {
  ...
  const isImageNote = video?.content_type === 'image'
  const primaryImage = video?.image_urls?.[0] || video?.cover_url || ''
  return (
    <AppShell full>
        ...
        {video ? (
          <div className="note-reader">
            <section className="media-panel">
              {isImageNote ? <img className="note-cover" src={primaryImage} alt={video.title} /> : <video className="note-cover" src={video.play_url} poster={video.cover_url} controls playsInline aria-label={video.title} />}
            </section>
